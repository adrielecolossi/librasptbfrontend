...
function ExibirOrdenarFrase() {
  const [isLoggedIn, setIsLoggedIn] = useState();
  const [apiResponse, setApiResponse] = useState([]);
  const [loadingEditar, setLoadingEditar] = useState(false);
  const [modalIsOpen, setIsOpen] = React.useState(false);
  const [fraseNovaQuestao, setFraseNovaQuestao] = useState("");
  ...
  const excluiQuestao = async (id) => {
    axiosClient
      .delete("/excluirQuestaoOrdenarFrase", {
        data: {
          token,
          id,
        }
  ...
  const editaQuestaoNoBanco = async (e) => {
    axiosClient
      .put("/editarQuestaoOrdenarFrase", {
        token,
        id,
        categoria: categoriasNovaQuestao,
        frase: fraseNovaQuestao,
      })
      .then((response) => {
        setLoadingEditar(false);
        setIsOpen(false);
  ...
  function openModal(id) {
    ...
    setFraseNovaQuestao(dadosDaQuestao.frase);
    setIsOpen(true);
  }
  function closeModal() {
    setIsOpen(false);
  ...
  if (isLoggedIn === "loggedIn" || isLoggedIn === undefined) {
    return (
      <>
        ...
        <Banner>
          <Title>
            <h1>Questões de Ordenar Frase</h1>
            <h2>
              Pesquise para ver todas as questões de ordenar a frase de uma dada
              categoria
            </h2>
          </Title>
          <img
            src="https://drive.google.com/uc?id=1hw9NhdbmhwB0jcaegsAXYkGYcHnnKUoe"
            alt=""
          />
        </Banner>
        <br></br>
        ...
        >
          ...
          <Div>
            <InputJS
              type="text"
              name="url"
              value={fraseNovaQuestao}
              style={{ marginTop: "3%", marginBottom: "5%" }}
              onChange={(v) => setFraseNovaQuestao(v.target.value)}
            ></InputJS>
          </Div>
          <div
            style={{
...
export default ExibirOrdenarFrase;
